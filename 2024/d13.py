...
def part_one(input):
    # tokens: a:3, b:1
    winning_combs = [_calc_presses(machine) for machine in input if _calc_presses(machine)]
    return sum([(press[0][0] * 3) + (press[0][1] * 1) for press in winning_combs])

def part_two(input):
    winning_combs = [_calc_presses_part_two(machine) for machine in input if _calc_presses_part_two(machine)]
...
if __name__ == "__main__":
    ...
    input = [_parse_input(machine.split("\n")) for machine in open("input/d13", "r").read().split("\n\n")]
